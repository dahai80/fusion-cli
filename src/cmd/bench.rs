...
async fn bench_mem(model: String) -> Result<()> {
    ...
    // 模拟加载模型
    println!();
    println!("  {} Loading model '{}' for memory measurement...", "⏳".blue(), model.cyan());
...
async fn bench_ctx(model: String, max_ctx: u32) -> Result<()> {
    ...
    println!("{}", "📊 Results".bold());
    println!("  Model:          {}", model.cyan());
    println!("  Max working ctx: {} tokens", max_working.to_string().cyan().bold());
...
async fn bench_auto(model: String) -> Result<()> {
    ...
    println!();
    println!("  {} Apply with: fusion config set mlx.default-ctx 4096", "💡".yellow());

    Ok(())
}

async fn bench_report(model: String, output: String) -> Result<()> {
    println!("{} Generating benchmark report for {}...", "📝".bold(), model.cyan());
